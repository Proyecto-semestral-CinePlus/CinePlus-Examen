...
public class PeliculaController {
    ...
    @Operation(summary = "Listar todas las películas",
            description = "Devuelve el catálogo completo con enlaces HATEOAS. Retorna 204 si no hay registros.")
    @ApiResponses({
    ...
    @Operation(summary = "Crear una nueva película",
            description = "Registra una película nueva. Los datos se validan con Bean Validation.")
    @ApiResponses({
    ...
    @PutMapping("/{id}")
    public ResponseEntity<EntityModel<Pelicula>> actualizarPelicula(@PathVariable int id, @Valid @RequestBody Pelicula pelicula) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<EntityModel<String>> eliminarPelicula(@PathVariable int id) {
    ...
    private EntityModel<Pelicula> ensamblarModelo(Pelicula pelicula) {
        return EntityModel.of(pelicula,
                ...
                linkTo(methodOn(PeliculaController.class).listarPeliculas()).withRel("peliculas"),
                linkTo(methodOn(PeliculaController.class).actualizarPelicula(pelicula.getId(), null)).withRel("actualizar"),
                linkTo(methodOn(PeliculaController.class).eliminarPelicula(pelicula.getId())).withRel("eliminar")
        );
    }
}